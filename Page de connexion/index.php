...
require("src/connection.php");
	...
	if(!empty($_POST['username']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['password_confirm'])){
		...
		$req = $db->prepare("SELECT count(*) as numberEmail FROM personnes WHERE email = ?");
	 	...
	 	<div id="form">
			<form method="POST" action="index.php">
				<table>
					<tr>
						<td>Username</td>
						<td><input type="text" name="username" placeholder="Ex : Rodrigue" autocomplete="off" required></td>
					</tr>
					<tr>
						<td>Email</td>
